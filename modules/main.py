    ...
    for topic_id in selected_topics:
        ...
        for data in topic_data:
            ...
            if not encrypted_url:
                await m.reply_text(f"No download link found for Topic ID {topic_id}.")
                continue

            # Log the encrypted URL
            logging.info(f"Encrypted URL for Topic ID {topic_id}: {encrypted_url}")
            ...
            try:
                ...
                await m.reply_text(f"Error decrypting URL for Topic ID {topic_id}: {str(e)}")
                continue
